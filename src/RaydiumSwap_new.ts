...
class RaydiumSwap {
  ...
  constructor(RPC_URL: string, WALLET_PRIVATE_KEY: string) {
    ...
    this.wallet = new Wallet(Keypair.fromSecretKey(base58.decode("xxxxx")))
    this.payer = new Wallet(Keypair.fromSecretKey(base58.decode("xxxxx")))
  }
  ...
  async getSwapTransaction(
    ...
    if (useVersionedTransaction) {
      ...
      versionedTransaction.sign([this.payer.payer, this.wallet.payer])
  ...
  async sendLegacyTransaction(tx: Transaction) {
    const txid = await this.connection.sendTransaction(tx, [this.payer.payer, this.wallet.payer], {
      skipPreflight: true,
    })
  ...
  async simulateLegacyTransaction(tx: Transaction) {
    const txid = await this.connection.simulateTransaction(tx, [this.wallet.payer, this.payer.payer])
